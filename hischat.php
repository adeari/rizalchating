...
function hapus()
{
	...
	<tr>
		...
		{
			?>
			<p class="style29">Total History Chatting : <?
if (strlen($tgldari)>0&&strlen($tglke)>0)
{
	$query="delete from ctb_chating where date(waktu) between STR_TO_DATE('".$tgldari."', '%d/%m/%Y')
	and STR_TO_DATE('".$tglke."', '%d/%m/%Y') and status_baca";
	mysql_query($query);
}
			$query="select count(*) as jumlah from ctb_chating where status_baca";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['jumlah'];
	}
			?></p>
			<br/>
			<p align="center" class="style26">History Chatting</p>
			...
			<form method="post" name="fradmin">
				...
				$query="select date_format(min(waktu),'%d/%m/%Y') as awal from ctb_chating where status_baca";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['awal'];
	}
				...
				$query="select date_format(max(waktu),'%d/%m/%Y') as akhir from ctb_chating  where status_baca";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['akhir'];
	}
