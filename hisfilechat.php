...
function hapus()
{
	...
	<tr>
		...
		{
			?>
			<p class="style29">Total File Chatting : <?
if (strlen($tgldari)>0&&strlen($tglke)>0)
{
	$query="select namafile from ctb_file where date(tglfle) between STR_TO_DATE('".$tgldari."', '%d/%m/%Y')
	and STR_TO_DATE('".$tglke."', '%d/%m/%Y')";
	$result = mysql_query($query);
	while ($row = mysql_fetch_array($result)) {
		@unlink("chat/".$row['namafile']);
	}
	$query="delete from ctb_file where date(tglfle) between STR_TO_DATE('".$tgldari."', '%d/%m/%Y')
	and STR_TO_DATE('".$tglke."', '%d/%m/%Y')";
	mysql_query($query);
}
			$query="select count(*) as jumlah from ctb_file";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['jumlah'];
	}
			?></p>
			<br/>
			<p align="center" class="style26">History File Chatting</p>
