...
function hapus()
{
	if (document.fradmin.tgldari.value<1) alert('Isi tanggal dari');
		...
		var answer = confirm("Apakah Broadcast akan di hapus ?")
		if (answer){
	...
	<tr>
		...
		{
			?>
			<p class="style29">Total Broadcast : <?
if (strlen($tgldari)>0&&strlen($tglke)>0)
{
	$query="select filenya from ctb_broadcast where date(tgl) between STR_TO_DATE('".$tgldari."', '%d/%m/%Y')
	and STR_TO_DATE('".$tglke."', '%d/%m/%Y')";
	$result = mysql_query($query);
	while ($row = mysql_fetch_array($result)) {
		@unlink("chat/uploadfile/".$row['filenya']);
	}
	$query="delete from ctb_broadcast where date(tgl) between STR_TO_DATE('".$tgldari."', '%d/%m/%Y')
	and STR_TO_DATE('".$tglke."', '%d/%m/%Y')";
	mysql_query($query);
}
			$query="select count(*) as jumlah from ctb_broadcast";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['jumlah'];
	}
			?></p>
			<br/>
			<p align="center" class="style26">BroadCast</p>
			...
			<form method="post" name="fradmin">
				...
				$query="select date_format(min(tglfle),'%d/%m/%Y') as awal from ctb_file";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['awal'];
	}
				...
				$query="select date_format(max(tglfle),'%d/%m/%Y') as akhir from ctb_file";
			$result = mysql_query($query);
	if ($row = mysql_fetch_array($result)) {
		echo $row['akhir'];
	}
